...
export class GeckoBootloaderClient {
  ...
  async connect({ timeout = 15000 } = {}) {
    ...
    const service = await this.#device.gatt.getPrimaryService(GECKO_BOOTLOADER_SERVICE_UUID);
  ...
  async writeFirmware(data, { reliable = false, wait = 10, progress, signal } = {}) {
    ...
    await this.#otaControlChar.writeValue(START_OTA);
    ...
    await this.#otaControlChar.writeValue(FINISH_OTA);
  }
